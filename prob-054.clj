;;;;;;;;;; problem 054 input ;;;;;;;;;;
(def prob-054-input [["8C" "TS" "KC" "9H" "4S" "7D" "2S" "5D" "3S" "AC"]
...
;;;;;;;;;; problem 054 ;;;;;;;;;;
(use '[clojure.contrib.seq-utils :only (includes? frequencies)])

(defn straights []
  (partition 5 1 (range 2 15)))
(def straights (memoize straights))

(defn count-freq [hand]
...
(defn straight? [hand]
  (includes? (straights) (map first hand)))

(defn royal-flush? [hand]
  (and (= (last (straights)) (map first hand))
       (flush? hand)))

(defn straight-flush? [hand]
  (and (straight? hand) (flush? hand)))

(defn filter-freq [hand n]
  (filter #(= n (last %)) (count-freq hand)))

(defn num-same [hand n]
  (count (filter-freq hand n)))
...
(defn full-house? [hand]
  (= [2 3] (sort (vals (count-freq hand)))))

(defn parse-hand [cards]
  (sort-by first (for [card (map #(rest (.split % "")) cards)
                       :let [rank (first card)
                             suit (last card)]]
                   (cond
                     (= rank "T") [10 suit]
...
(defn high-card [h1 h2]
  (if (empty? h1)
    "tie"
    (let [c1 (first (last h1)) c2 (first (last h2))]
      (cond
        (> c1 c2) "p1"
        (< c1 c2) "p2"
        :else (recur (butlast h1) (butlast h2))))))

(defn prob-054 []
  (frequencies (for [hands (map #(partition 5 %) prob-054-input)
                     :let [h1 (parse-hand (first hands))
                           h2 (parse-hand (last hands))
                           v1 (hand-val h1)
                           v2 (hand-val h2)]]
                 (cond
                   (> v1 v2) "p1"
                   (< v1 v2) "p2"
                   (= v1 v2 1) (let [c1 (ffirst (filter-freq h1 2))
                                     c2 (ffirst (filter-freq h2 2))]
                                 (cond
                                   (> c1 c2) "p1"
                                   (< c1 c2) "p2"
                                   :else (high-card h1 h2)))
                   :else (high-card h1 h2)))))